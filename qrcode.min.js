...
!(() => {
  ...
  ;(a.prototype = {
    ...
    (b.createData = (a, c, d) => {
      ...
      for (g.getLengthInBits() + 4 <= 8 * l && g.put(0, 4); 0 != g.getLengthInBits() % 8; ) g.putBit(!false)
      for (;;) {
    ...
    (b.createBytes = (a, b) => {
      for (var c = 0, d = 0, e = 0, g = new Array(b.length), h = new Array(b.length), _j = 0; _j < b.length; _j++) {
        var k = b[_j].dataCount,
          l = b[_j].totalCount - k
        ;(d = Math.max(d, k)), (e = Math.max(e, l)), (g[_j] = new Array(k))
        for (var m = 0; m < g[_j].length; m++) g[_j][m] = 255 & a.buffer[m + c]
        c += k
        var n = f.getErrorCorrectPolynomial(l),
          o = new i(g[_j], n.getLength() - 1),
          p = o.mod(n)
        h[_j] = new Array(n.getLength() - 1)
        for (var m = 0; m < h[_j].length; m++) {
          var q = m + p.getLength() - h[_j].length
          h[_j][m] = q >= 0 ? p.get(q) : 0
        }
      }
      for (var r = 0, m = 0; m < b.length; m++) r += b[m].totalCount
      for (var s = new Array(r), t = 0, m = 0; d > m; m++)
        for (var _j = 0; _j < b.length; _j++) m < g[_j].length && (s[t++] = g[_j][m])
      for (var m = 0; e > m; m++) for (var _j = 0; _j < b.length; _j++) m < h[_j].length && (s[t++] = h[_j][m])
      return s
    })
  for (
      ...
      f = {
        ...
        getLostPoint: (a) => {
          for (var b = a.getModuleCount(), c = 0, d = 0; b > d; d++)
            for (var e = 0; b > e; e++) {
              for (var f = 0, ga = a.isDark(d, e), h = -1; 1 >= h; h++)
                if (!(0 > d + h || d + h >= b))
                  for (var i = -1; 1 >= i; i++)
                    0 > e + i || e + i >= b || ((0 != h || 0 != i) && ga == a.isDark(d + h, e + i) && f++)
              f > 5 && (c += 3 + f - 5)
            }
          for (var d = 0; b - 1 > d; d++)
            for (var e = 0; b - 1 > e; e++) {
    ...
    g.EXP_TABLE[_ga] = 1 << _ga
  for (var _ga = 8; 256 > _ga; _ga++)
    g.EXP_TABLE[_ga] = g.EXP_TABLE[_ga - 4] ^ g.EXP_TABLE[_ga - 5] ^ g.EXP_TABLE[_ga - 6] ^ g.EXP_TABLE[_ga - 8]
  for (var _ga = 0; 255 > _ga; _ga++) g.LOG_TABLE[g.EXP_TABLE[_ga]] = _ga
  ;(i.prototype = {
    get: function (a) {
      return this.num[a]
    },
    ...
    o = (() => {
      ...
      return (
        (a.prototype.draw = function (a) {
          ...
          var h = g("svg", { viewBox: "0 0 " + String(d) + " " + String(d), width: "100%", height: "100%" }),
            i = g("rect", { fill: b.colorLight, width: "100%", height: "100%" }),
            j = g("g", { fill: b.colorDark, "shape-rendering": "crispEdges" })
          h.appendChild(i)
          for (var k = 0; d > k; k++)
            for (var l = 0; d > l; l++)
              if (a.isDark(k, l)) {
                var m = g("rect", { x: String(l), y: String(k), width: "1", height: "1" })
                j.appendChild(m)
              }
          h.appendChild(j)
